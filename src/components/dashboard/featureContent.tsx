...
const atendimento: FeatureContentDef = {
  statusTitle: "Agente Ativo",
  statusSub: "A funcionar — última ação há 2 min",
  kpis: [
    { label: "Mensagens Respondidas Hoje", value: "47" },
    { label: "Esta Semana", value: "312" },
    { label: "Taxa de Resposta", value: "99%" },
  ],
  render: () => (
    <>
      <ActivityList items={[
      ...
      <div className="mt-6">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {[
                { p: "Ana Silva", h: "14:32", d: "2 min", s: "respondido" },
                { p: "João Costa", h: "14:18", d: "5 min", s: "escalado" },
                { p: "Maria Santos", h: "14:10", d: "1 min", s: "respondido" },
                { p: "Pedro Ferreira", h: "13:47", d: "3 min", s: "respondido" },
                { p: "Carla Mendes", h: "13:22", d: "2 min", s: "respondido" },
              ].map((r, i) => (
                <tr key={i} className="border-b border-border/50">
                  <td className="py-2">{r.p}</td><td className="py-2">{r.h}</td><td className="py-2">{r.d}</td><td className="py-2"><StatusBadge status={r.s} /></td>
                </tr>
              ))}
            </tbody>
          </table>
...
const agendamento: FeatureContentDef = {
  statusTitle: "Agente Ativo",
  statusSub: "A funcionar — último agendamento há 8 min",
  kpis: [
    { label: "Agendamentos Hoje", value: "8" },
    { label: "Esta Semana", value: "34" },
    { label: "Taxa de Confirmação", value: "94%" },
  ],
  render: () => {
    const days = ["Seg", "Ter", "Qua", "Qui", "Sex"];
    const slots = [3, 5, 2, 4, 6];
    return (
      <>
        <ActivityList items={[
            ...
            {days.map((d, i) => (
              <div key={d} className="text-center">
                ...
                <div className="bg-muted rounded-lg p-3 space-y-1">
                  {Array.from({ length: slots[i] }).map((_, j) => (
                    <div key={j} className="w-2 h-2 rounded-full bg-primary mx-auto" />
                  ))}
                </div>
...
const lembretes: FeatureContentDef = {
  statusTitle: "Agente Ativo",
  statusSub: "A funcionar — último lembrete há 12 min",
  kpis: [
    { label: "Lembretes Enviados Hoje", value: "12" },
...
const relatorios: FeatureContentDef = {
  statusTitle: "Relatório de Março disponível",
  statusSub: "Gerado automaticamente em 01/04/2026",
  kpis: [
    { label: "Consultas no Mês", value: "142" },
    { label: "Taxa Média de Comparência", value: "87%" },
    { label: "Receita Estimada", value: "€4.260" },
  ],
  render: () => {
    const weekData = [
      { name: "Sem 1", consultas: 32 },
      { name: "Sem 2", consultas: 38 },
      { name: "Sem 3", consultas: 35 },
      { name: "Sem 4", consultas: 37 },
    ];
    return (
      <>
        <div className="space-y-3 mb-6">
          ...
          {["Março 2026", "Fevereiro 2026", "Janeiro 2026"].map((m, i) => (
            <div key={i} className="flex items-center justify-between p-3 rounded-lg bg-secondary/50">
              ...
              <Button variant="outline" size="sm" disabled>Ver Relatório</Button>
            </div>
          ))}
        </div>
        <div>
          ...
          <div className="h-48">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={weekData}>
                ...
                <Tooltip contentStyle={{ background: "hsl(220,30%,8%)", border: "1px solid hsl(220,20%,18%)", borderRadius: 8 }} />
                <Bar dataKey="consultas" fill="hsl(216,100%,56%)" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
...
const prioridade: FeatureContentDef = {
  statusTitle: "Agente Ativo",
  statusSub: "Triagem em tempo real",
  kpis: [
    { label: "Urgências Detectadas Hoje", value: "3" },
...
const followup: FeatureContentDef = {
  statusTitle: "Agente Ativo",
  statusSub: "Follow-ups automáticos",
  kpis: [
    { label: "Follow-ups Enviados Hoje", value: "6" },
    { label: "Esta Semana", value: "28" },
    { label: "Taxa de Retenção", value: "91%" },
  ],
  render: () => {
    const timeline = [
      { label: "D+1 Pós-Consulta", done: true },
      { label: "D+30 Reengagement", done: true },
      { label: "D+90 Reengagement", done: false },
    ];
    return (
      <>
        <div className="mb-6">
          ...
          <div className="flex items-center gap-2">
            {timeline.map((t, i) => (
              <div key={i} className="flex items-center gap-2">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${t.done ? "bg-emerald-500/20 text-emerald-400" : "bg-muted text-muted-foreground"}`}>
                  {t.done ? "✓" : "—"}
                </div>
...
const secChamadas: FeatureContentDef = {
  statusTitle: "Secretária V3",
  statusSub: "Chamadas ativas",
  kpis: [
    { label: "Chamadas Hoje", value: "5" },
    { label: "Esta Semana", value: "19" },
    { label: "Resolvidas pela IA", value: "84%" },
  ],
  render: () => (
    <div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-border text-muted-foreground text-left">
            <th className="pb-2 font-medium">Hora</th><th className="pb-2 font-medium">Duração</th><th className="pb-2 font-medium">Resultado</th>
          </tr></thead>
          <tbody>
            {[
              { h: "14:45", d: "3:20", r: "resolvido" },
              { h: "13:12", d: "1:45", r: "resolvido" },
              { h: "11:30", d: "5:10", r: "transferido" },
              { h: "10:05", d: "2:30", r: "resolvido" },
              { h: "09:22", d: "4:15", r: "resolvido" },
            ].map((r, i) => (
              <tr key={i} className="border-b border-border/50">
                <td className="py-2">{r.h}</td><td className="py-2">{r.d}</td><td className="py-2"><StatusBadge status={r.r} /></td>
              </tr>
            ))}
          </tbody>
        </table>
...
const secPagamentos: FeatureContentDef = {
  statusTitle: "Verificação de pagamentos ativa",
  statusSub: "Última verificação há 5 min",
  kpis: [
    { label: "Verificações Hoje", value: "8" },
    { label: "Esta Semana", value: "34" },
    { label: "Taxa de Sucesso", value: "96%" },
  ],
  render: () => (
    <div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-border text-muted-foreground text-left">
            <th className="pb-2 font-medium">Paciente</th><th className="pb-2 font-medium">Valor</th><th className="pb-2 font-medium">Método</th><th className="pb-2 font-medium">Estado</th>
          </tr></thead>
          <tbody>
            {[
              { p: "Ana Silva", v: "€45", m: "MBway", s: "confirmado" },
              { p: "João Costa", v: "€60", m: "MBway", s: "confirmado" },
              { p: "Maria Santos", v: "€45", m: "MBway", s: "pendente" },
              { p: "Pedro Reis", v: "€75", m: "MBway", s: "confirmado" },
              { p: "Carla Mendes", v: "€45", m: "MBway", s: "confirmado" },
            ].map((r, i) => (
              <tr key={i} className="border-b border-border/50">
                <td className="py-2">{r.p}</td><td className="py-2">{r.v}</td><td className="py-2">{r.m}</td><td className="py-2"><StatusBadge status={r.s} /></td>
              </tr>
            ))}
          </tbody>
        </table>
...
const secCalendar: FeatureContentDef = {
  statusTitle: "Calendário sincronizado",
  statusSub: "Sem conflitos",
  kpis: [
    { label: "Conflitos Evitados Esta Semana", value: "7" },
    { label: "Consultas Agendadas Esta Semana", value: "34" },
  ...
  render: () => {
    ...
    const filled: Record<string, string[]> = {
      Seg: ["09:00", "11:00", "15:00"],
      Ter: ["10:00", "14:00", "16:00"],
      Qua: ["09:00", "10:00", "11:00", "15:00"],
      Qui: ["14:00", "16:00", "17:00"],
      Sex: ["09:00", "11:00"],
    };
    return (
      <div>
        ...
        <div className="overflow-x-auto">
          <div className="grid grid-cols-6 gap-1 min-w-[400px]">
            ...
            {hours.map(h => (
              <>
                <div key={`h-${h}`} className="text-xs text-muted-foreground py-1 text-right pr-2">{h}</div>
                {days.map(d => (
                  <div key={`${d}-${h}`} className={`rounded h-6 ${filled[d]?.includes(h) ? "bg-primary/30" : "bg-muted/30"}`} />
                ))}
              </>
            ))}
          </div>
...
const secLembretes: FeatureContentDef = {
  statusTitle: "Lembretes e follow-ups avançados ativos",
  statusSub: "Última sequência há 15 min",
  kpis: [
    { label: "Enviados Hoje", value: "18" },
    { label: "Esta Semana", value: "94" },
    { label: "Taxa de Abertura", value: "87%" },
  ],
  render: () => {
    const seq = [
      { label: "D-2", done: true },
      { label: "D-1", done: true },
      { label: "D+1", done: true },
      { label: "D+30", done: false },
      { label: "D+90", done: false },
    ];
    return (
      <div>
        ...
        <div className="flex items-center gap-1 flex-wrap mb-6">
          {seq.map((s, i) => (
            <div key={i} className="flex items-center gap-1">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold ${s.done ? "bg-emerald-500/20 text-emerald-400" : "bg-muted text-muted-foreground"}`}>
                {s.label}
              </div>
...
const secEscalamento: FeatureContentDef = {
  statusTitle: "Escalamento inteligente ativo",
  statusSub: "Última escalação há 20 min",
  kpis: [
    { label: "Escalações Este Mês", value: "12" },
    { label: "Resolvidas", value: "10" },
    { label: "Tempo Médio de Resolução", value: "8 min" },
  ],
  render: () => {
    const [tab, setTab] = useState("todos");
    const items = [
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsContent value={tab}>
          <div className="overflow-x-auto mt-4">
            <table className="w-full text-sm">
              <thead><tr className="border-b border-border text-muted-foreground text-left">
                <th className="pb-2 font-medium">Paciente</th><th className="pb-2 font-medium">Motivo</th><th className="pb-2 font-medium">Hora</th><th className="pb-2 font-medium">Estado</th><th className="pb-2" />
              </tr></thead>
              <tbody>
                {filtered.map((r, i) => (
                  <tr key={i} className="border-b border-border/50">
                    <td className="py-2">{r.p}</td><td className="py-2">{r.motivo}</td><td className="py-2">{r.time}</td><td className="py-2"><StatusBadge status={r.status} /></td>
                    <td className="py-2"><Button variant="ghost" size="sm" disabled>Ver</Button></td>
                  </tr>
                ))}
              </tbody>
            </table>
...
const secFinanceiro: FeatureContentDef = {
  statusTitle: "Relatórios financeiros automáticos",
  statusSub: "Atualizado há 1h",
  kpis: [
    { label: "Receita Este Mês", value: "€5.840" },
    { label: "vs Mês Anterior", value: "+12%" },
    { label: "Consultas Faturadas", value: "67" },
  ],
  render: () => {
    const monthData = [
      { name: "Out", receita: 4200 },
      { name: "Nov", receita: 4600 },
      { name: "Dez", receita: 4100 },
      { name: "Jan", receita: 4800 },
      { name: "Fev", receita: 5200 },
      { name: "Mar", receita: 5840 },
    ];
    const tableData = [
      { mes: "Março", consultas: 67, receita: "€5.840", crescimento: "+12%" },
      { mes: "Fevereiro", consultas: 58, receita: "€5.200", crescimento: "+13%" },
      { mes: "Janeiro", consultas: 52, receita: "€4.800", crescimento: "+17%" },
      { mes: "Dezembro", consultas: 48, receita: "€4.100", crescimento: "-11%" },
    ];
    return (
      <>
        <div className="mb-6">
          ...
          <div className="h-48">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={monthData}>
                ...
                <YAxis stroke="hsl(215,15%,60%)" fontSize={12} />
                <Tooltip contentStyle={{ background: "hsl(220,30%,8%)", border: "1px solid hsl(220,20%,18%)", borderRadius: 8 }} />
                <Line type="monotone" dataKey="receita" stroke="hsl(216,100%,56%)" strokeWidth={2} dot={{ fill: "hsl(216,100%,56%)" }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-border text-muted-foreground text-left">
              <th className="pb-2 font-medium">Mês</th><th className="pb-2 font-medium">Consultas</th><th className="pb-2 font-medium">Receita</th><th className="pb-2 font-medium">Crescimento</th>
            </tr></thead>
            <tbody>
              {tableData.map((r, i) => (
                <tr key={i} className="border-b border-border/50">
                  <td className="py-2">{r.mes}</td><td className="py-2">{r.consultas}</td><td className="py-2">{r.receita}</td>
                  <td className={`py-2 ${r.crescimento.startsWith("+") ? "text-emerald-400" : "text-red-400"}`}>{r.crescimento}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
export const featureContentMap: Record<string, FeatureContentDef> = {
  ...
  prioridade,
  followup,
  "sec-chamadas": secChamadas,
  "sec-pagamentos": secPagamentos,
  "sec-calendar": secCalendar,
  "sec-lembretes": secLembretes,
  "sec-escalamento": secEscalamento,
  "sec-financeiro": secFinanceiro,
};
